...
public class BufferedNetworkStoreClient extends AbstractForwardingNetworkStoreClient<RestNetworkStoreClient> {
    ...
    private final NetworkCollectionIndex<OperationalLimitsGroupsRemovalBuffer<?>> operationalLimitsToFlush =
            new NetworkCollectionIndex<>(() -> new OperationalLimitsGroupsRemovalBuffer<>(delegate));

    private final NetworkCollectionIndex<ExtensionsRemovalBuffer<?>> extensionsToFlush =
            new NetworkCollectionIndex<>(() -> new ExtensionsRemovalBuffer<>(delegate));
    ...
    public void removeExtensionsAttributes(UUID networkUuid, int variantNum, ResourceType resourceType, Map<String, Set<String>> extensionsByIdentifiableId) {
        ...
        extensionsToFlush.getCollection(networkUuid, variantNum).remove(finalExtensionsByIdentifiableIds, resourceType);
    }
