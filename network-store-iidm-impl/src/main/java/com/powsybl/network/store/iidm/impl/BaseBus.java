...
public interface BaseBus extends Bus {
    ...
    @Override
    default Iterable<BoundaryLine> getBoundaryLines(BoundaryLineFilter boundaryLineFilter) {
        return getBoundaryLineStream(boundaryLineFilter).collect(Collectors.toList());
    }

    @Override
    default Stream<BoundaryLine> getBoundaryLineStream(BoundaryLineFilter boundaryLineFilter) {
        return getBoundaryLineStream().filter(boundaryLineFilter.getPredicate());
    }
